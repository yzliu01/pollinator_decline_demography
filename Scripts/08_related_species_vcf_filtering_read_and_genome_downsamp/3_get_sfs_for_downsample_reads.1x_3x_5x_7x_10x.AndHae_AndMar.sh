...
#SBATCH --error=3_Hae_Mar_downsample_read_2025_DP_1x_3x_5x_7x_10x.%A_%a.e
#SBATCH --output=3_Hae_Mar_downsample_read_2025_DP_1x_3x_5x_7x_10x.%A_%a.o
#SBATCH --job-name=3_Hae_Mar_downsample_read_2025_DP_1x_3x_5x_7x_10x
...
Andhae.New_REF_AndHae_0_4.100kb_1500x_region.clip.vcf.gz # above have 0 sfs, run them with modified script
...
## manual change file names
## for 100% reads {AndHae,AndMar} from genome sampling
#/home/yzliu/eDNA/faststorage/yzliu/DK_proj/population_genomics/bee_proj_script/data_process/data_2025/steps/07_sfs_snp_counts_stairway_plot_steps_after_vcf/downsample_genome_reads_and_2025_new_pools_prepare_plots/
#1_downsample_genome_reads_sfs_calculation_snp_counts.removed_contamination.sh
...
#vcf_file=$(ls Andmar.New_REF_AndMar_*.100kb_1500x_region.clip.vcf.gz | sort -V | sed -n ${SLURM_ARRAY_TASK_ID}p)
vcf_file=$(echo ${vcf_list[@]} | tr ' ' '\n' | sed -n ${SLURM_ARRAY_TASK_ID}p)
## Andmar.New_REF_AndMar_0_1.100kb_1500x_region.vcf.gz
output_prefix=${vcf_file/.100kb_1500x_region.clip.vcf.gz/}
## Andmar.New_REF_AndMar_0_1.
...
#done
    ...
    ##awk '$1=$1'| cut -d ' ' -f 1 | tr '\n' ' ' > $output_SFS_dir/"$output_prefix"P_downsample_reads."${depth_time[i]}".equal_self.sfs
    ## output file name example
    #Andmar.New_REF_AndMar_0_1.P_downsample_reads.1x.equal_self.sfs
    #Andmar.New_REF_AndMar_0_1.P_downsample_reads.3x.equal_self.sfs
